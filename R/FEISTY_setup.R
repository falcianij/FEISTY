...
setupVerticalO2 <- function(profile, szprod = NULL, lzprod = NULL, bprodin = NA, dfbot = NA, dfpho = NA,
  nStages = 9, photic = NULL, shelfdepth = 250, visual = 1.5, etaMature = 0.25, Fmax = 0, etaF = 0.05,
  bET = TRUE, I_eu = 0.01, ssigma = 10, tau = 10, use_legacy_visual = FALSE,
  K_O = 2, h_O = 1, T_ref_O2 = 10, b_D = 0.75, b_S = 0.67,
  Q10_D = 2, Q10_S = 1.5, delta_pO2_ref = 2, w_ref_O2 = 1, use_oxygen = TRUE,
  use_light = TRUE, resource_input_units = "molN_m3", input_to_gN = 14.0, N_to_C = 5.625, C_to_wet = 10, ...) {
  required_cols <- c(
  ...
  normalize_mass_profile <- function(x, fallback) {
    x[!is.finite(x) | x < 0] <- 0
    total <- sum(x)
    if (total > 0) x / total else fallback
  }
  vertical_profile <- function(z_mid, dz, sigma, xloc) {
    xloc <- rep(xloc, length.out = length(sigma))
  ...
  small_day <- profile$zmicro_day * conv * dz; small_night <- profile$zmicro_night * conv * dz
  large_day <- profile$zmeso_day * conv * dz; large_night <- profile$zmeso_night * conv * dz
  if (is.null(szprod)) szprod <- 0.5 * (sum(small_day) + sum(small_night)); if (is.null(lzprod)) lzprod <- 0.5 * (sum(large_day) + sum(large_night))
  fallback <- ifelse(profile$I_day_rel >= I_eu, 1, 0); if (sum(fallback) == 0) fallback[] <- 1; fallback <- fallback/sum(fallback)
  P_small_day <- normalize_mass_profile(small_day, fallback); P_small_night <- normalize_mass_profile(small_night, fallback)
  P_large_day <- normalize_mass_profile(large_day, fallback); P_large_night <- normalize_mass_profile(large_night, fallback)
  ...
  param$depthDay[,1] <- P_small_day; param$depthDay[,2] <- P_large_day; param$depthDay[,3] <- P_benthos; param$depthDay[,4] <- P_spare
  param$depthNight[,1] <- P_small_night; param$depthNight[,2] <- P_large_night; param$depthNight[,3] <- P_benthos; param$depthNight[,4] <- P_spare
  ix <- param$ix[[1]]; param$depthDay[,ix] <- vertical_profile(z_mid,dz,sigmap[ix],surface_depth); param$depthNight[,ix] <- param$depthDay[,ix]
  ix <- param$ix[[2]]; param$depthNight[,ix] <- vertical_profile(z_mid,dz,sigmap[ix],surface_depth); param$depthDay[,ix] <- vertical_profile(z_mid,dz,sigmap[ix],dvm_depth)
  ix <- param$ix[[3]]; flags <- get_stage_flags(ix); pn <- vertical_profile(z_mid,dz,sigmap[ix],surface_depth); xd <- rep(surface_depth,length(ix)); xd[flags$lg] <- dvm_depth; pd <- vertical_profile(z_mid,dz,sigmap[ix],xd); param$depthNight[,ix] <- pn; param$depthDay[,ix] <- 0.5*pd+0.5*pn
  ix <- param$ix[[4]]; flags <- get_stage_flags(ix); xn <- rep(surface_depth,length(ix)); xn[flags$lg] <- dvm_depth; param$depthNight[,ix] <- vertical_profile(z_mid,dz,sigmap[ix],xn); param$depthDay[,ix] <- vertical_profile(z_mid,dz,sigmap[ix],dvm_depth)
  ix <- param$ix[[5]]; flags <- get_stage_flags(ix); xn <- rep(surface_depth,length(ix)); xn[flags$med] <- bottom_target; xd <- xn; xd[flags$lg] <- demmig_depth; dn <- vertical_profile(z_mid,dz,sigmap[ix],xn); dd <- vertical_profile(z_mid,dz,sigmap[ix],xd); if(bottom <= photic){dd <- 0.5*(dd+dn); dn <- dd}; param$depthNight[,ix] <- dn; param$depthDay[,ix] <- dd
  phi_day <- pmax(0, profile$I_day_rel); phi_night <- pmax(0, profile$I_night_rel)
  if (!isTRUE(use_light)) { phi_day[] <- 1; phi_night[] <- 1 }
  ...
  ix_dem <- param$ix[[5]]; ml_dem <- param$mLower[ix_dem]; ixmedium <- which.min(abs(ml_dem - 0.5)); ixlarge <- which.min(abs(ml_dem - 250)); param$ixmedium <- ixmedium; param$ixlarge <- ixlarge
  safe_seq <- function(a, b) if (is.finite(a) && is.finite(b) && b >= a) seq.int(a, b) else integer(0)
  idx_be <- safe_seq(param$ixFish[1], ix_dem[1] + (ixmedium - 2)); if (length(idx_be) > 0) { param$theta[idx_be, 3:4] <- 0; if (ixlarge >= (ixmedium + 1)) { idx_smd <- safe_seq(ix_dem[1] + (ixmedium - 1), ix_dem[1] + (ixlarge - 2)); param$theta[idx_be, idx_smd] <- param$theta[idx_be, idx_smd] * 0.25 } }
  pred1 <- safe_seq(param$ix[[3]][1] + (ixlarge - 1), param$ix[[3]][length(param$ix[[3]])]); pred2 <- safe_seq(param$ix[[4]][1] + (ixlarge - 1), param$ix[[4]][length(param$ix[[4]])]); pred3 <- safe_seq(ix_dem[1] + (ixlarge - 1), ix_dem[length(ix_dem)])
  prey1 <- safe_seq(param$ix[[1]][1] + (ixmedium - 1), param$ix[[1]][length(param$ix[[1]])]); prey2 <- safe_seq(param$ix[[2]][1] + (ixmedium - 1), param$ix[[2]][length(param$ix[[2]])])
  idx_predat <- c(pred1, pred2, pred3); idx_prey <- c(prey1, prey2); if (length(idx_predat) > 0 && length(idx_prey) > 0) param$theta[idx_predat, idx_prey] <- param$theta[idx_predat, idx_prey] * 0.5
  ...
  glvl_eff <- rep(1, param$nStages); for (i in param$ixFish) glvl_eff[i] <- 0.5*sum(param$depthDay[,i]*glvl_local[,i]) + 0.5*sum(param$depthNight[,i]*glvl_local[,i])
  if (!isTRUE(use_oxygen)) { glvl_local[,] <- 1; glvl_eff[] <- 1 }
  ...
  param$z_mid <- z_mid; param$z_top <- z_top; param$z_bot <- z_bot; param$dz <- dz; param$phiLightDay <- phi_day; param$phiLightNight <- phi_night; param$profile <- profile
