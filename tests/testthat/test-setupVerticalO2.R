library(FEISTY)
source(test_path('../../scripts/vertical_o2_profiles.R'))
...
test_that('profile loader handles trailing NA and internal NA', {
  ...
  d <- data.frame(site='A', scenario='hist', depth_idx=1:4, depth_mid_m=c(5,15,25,35), depth_top_m=c(0,10,20,30), depth_bot_m=c(10,20,30,40), dz_m=10,
                  temp_C=c(10,9,8,NA), pO2_kPa=c(8,7,6,NA), zmeso_day=c(1,1,1,NA), zmeso_night=c(1,1,1,NA), zmicro_day=c(2,2,2,NA), zmicro_night=c(2,2,2,NA), I_day_rel=c(1,0.5,0.1,NA), I_night_rel=c(0.1,0.1,0.05,NA))
...
test_that('setupVerticalO2 depth arrays, light and oxygen are bounded', {
  ...
  expect_equal(p$phiLightDay, p$profile$I_day_rel)
  expect_equal(p$phiLightNight, p$profile$I_night_rel)
...
test_that('setupVerticalO2 can disable oxygen limitation without changing profile pO2', {
  p <- setupVerticalO2(profile=read_vertical_o2_profile(site='CCE', scenario='hist'), nStages=9, use_oxygen=FALSE)
